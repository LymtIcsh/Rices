...
namespace Suture
{
    public class EntityLoader : IReference
    {
        ...
        public int ShowEntity<T>(Action<Entity> onShowSuccess, string entityGroup,
            int ArmorAsset, EntityData entityData, object userData = null)
            where T : EntityLogic
        {
            int serialId = GameEntry.Entity.GenerateSerialId();
            dicCallback.Add(serialId, onShowSuccess);
            GameEntry.Entity.ShowEntity<T>(entityGroup, ArmorAsset, entityData);
            return serialId;
        }
        ...
        public void HideEntity(int serialId)
        {
            ...
            if (!dicSerial2Entity.TryGetValue(serialId, out entityBase))
            {
                Log.Error("没有找到实体('serial id:{0}') ", serialId);
            }

            dicSerial2Entity.Remove(serialId);
            dicCallback.Remove(serialId);

            Entity[] entities = GameEntry.Entity.GetChildEntities(entityBase);
            if (entities != null)
            {
            ...
            GameEntry.Entity.HideEntity(entityBase);
        }

        public void HideEntity(Entity entityBase)
        {
            if (entityBase == null)
                return;

            HideEntity(entityBase.Id);
        }
        ...
        public void HideAllEntity()
        {
            ...
            {
                Entity parentEntityBase = GameEntry.Entity.GetParentEntity(entity);
                //有ParentEntity
                if (parentEntityBase != null)
                {
                    //若Parent Entity由这个Loader对象托管，则把这个Child Entity从数据中移除，在隐藏Parent Entity，GF内部会处理Child Entity
                    if (dicSerial2Entity.ContainsKey(parentEntityBase.Id))
                    {
